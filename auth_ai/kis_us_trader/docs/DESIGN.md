...
| 2026-06-09 | **주문 타이밍 실증 확정**: `test/test_offhours_order.py`로 미국 완전 마감 시각 paper 매수 전송 → 거부 `rt_cd=1 / msg_cd=40580000 / "모의투자 장종료"`. **paper도 장외 거부**(체결 시뮬레이션 안 함) → 결함 prod 한정 아님, 07:30 매매 시도 시 매번 거부+`consecutive_errors`↑(3회면 auto-pause). 1주 운영 무사는 hold/skip 으로 주문 단계 미도달 덕. 남은 불확실: KIS 미국 예약주문 지원. ORDER_TIMING_ISSUE.md 갱신. |
| 2026-06-10 | **P1 백테스트 + 검증**: 2년×5종목, signal_strength 방향 엣지 미검출(P1a), 프롬프트 변경은 P&L 중립=안전성 수정(P1b), 타이밍 알파 없음·하락방어만(P1c), 방향 베팅 손실(P1d). throttle이지 알파 아님 재확인. SIGNAL_STRENGTH_ANALYSIS.md. |
| 2026-06-10 | **주문 타이밍 Rank 2 구현(승인-제출 분리)**: KIS 미국 예약주문 지원 확인(공식 repo, `VTTT3014U` 등)했으나 07:30이 접수창(10:00~) 밖 + 모의 조회 불가 → Rank 1 보류, **Rank 2 채택**. 07:30 승인 → `state.pending_orders` 저장 → `submission_loop`가 미국 개장 직후(09:35 ET≈22:35 KST) 검증 정규 TR로 제출 + 잔고확인 체결. 18h TTL 만료. 텔레그램 메시지에 추세+LLM 사유 추가. ORDER_TIMING_ISSUE.md. |
| 2026-06-10 | **Rank 2 정규장 제출 실증 완료**: `test/test_rank2_pending.py RUN` 정규장(22:30 KST=09:30 ET 개장 직후) 실행 — Part 1(pending state 라운드트립) + Part 2(submit_open_orders 통합 실주문). 결과: BUY AAPL 1주 @ $292.25 → ODNO **0000039098** 접수 → `📊 매수 체결 — 보유 1주` 잔고확인 통과 → 안전 청산 SELL @ $289.35 ODNO **0000039135** 접수 → KIS 앱에서 체결 확인(보유 0주 복귀, 잔여 없음). 봇과 동일 경로(`_submit_one`/`_place_and_confirm`/`apply_fill`)로 pending→submit→체결확인 end-to-end 검증, phantom-fill 방지 잔고확인 정상 동작. **Rank 2 = 운용 가능**. ORDER_TIMING_ISSUE.md. |
| 2026-06-10 | **P1-후속 완료(flipped-day P&L · strong-down EXIT 시뮬)**: `tools/flipped_day_pnl.py` 신규(Yahoo 2y×5종목, 분석 전용). 표본 OLD weak 0.9%/NEW weak 16.0%/flipped 363일 P1 문서와 일치. ① flipped uptrend 5d mean **+1.089%(t=+2.27)** vs non-flipped uptrend **+0.962%** — 차이 +0.13%p=시장평균 → **신 weak 룰은 throttle, 알파/손실 둘 다 아님**(다중검정·Newey-West 보정 시 t 붕괴). ② strong-down EXIT vs HOLD 시뮬 **5종목 일관 −22~−82%p**(평균 Δ=−59.77%p) → 구 "strong→매도" 넛지 제거 정당화(P1d 위해경로 직접 측정으로 재확인). P1b 항등식→측정 전환 성공. SIGNAL_STRENGTH_ANALYSIS.md. |
| 2026-06-10 | **P2 완료(종목별/변동성 정규화 임계값)**: `kis/signals.py` 에 `BASELINE_VOL=0.015`(=AAPL p50 trailing 20d vol round), `VOL_WINDOW=20`, `compute_vol_factor(closes)` 도입 + `classify_strength(spread, chg, vol_factor=1.0)` 으로 score 정규화. `daily_trader.compute_trend` 가 closes 로 vol_factor 산출·전달 + trend dict 에 노출. `tools/tune_thresholds.py` 도 동일 경로(드리프트 차단). **종목별 weak 발동률**: AAPL 31%/NVDA 16%/AMD 8%/TSM 13%/INTC 11% (range 23.3pp) → AAPL 27%/NVDA 35%/AMD 31%/TSM 36%/INTC 39% (range 12.3pp, **~2배 균일화**). 덤: AMD strong 과발동(~64%) 도 norm score 우선 판정으로 동시 throttle. 호환성: vol_factor=1.0 기본값 → P1 호출 회귀 0 (`test/test_signals.py` 23 케이스 + `test/test_safety_gate.py` 22 케이스 통과). 측정 도구 `tools/vol_calibration.py` 신규. SIGNAL_STRENGTH_ANALYSIS.md P2 절. |
| 2026-06-11 | **Phase 2 코드 골격 완료(오프라인 테스트 green, 운영 검증 대기)**: ① `kis/universe.py` 반도체 10종목 추가(NVDA/AMD/AVGO/MU/INTC/QCOM/TXN/AMAT/LRCX/TSM, TSM만 NYSE) → 화이트리스트 11종목. ② `sector.py` 신규 — `compute_macro_bias(client, member_above_sma20)` = SMH 일봉 **1콜** + 유니버스 breadth(종목 above_sma20 주입, 추가 호출 0) → `classify_bias`(상승정렬+breadth≥60→risk_on / 하락정렬·breadth≤40→risk_off / 혼조→neutral / 조회실패→unknown, **risk_off 우선 보수**) + `max_buys_for_bias`(N=3/2/0/0). ③ `llm_advisor.py` — `sanitize_advice`(순수: action/conf 정규화 + **reason 에 다른 화이트리스트 ticker 섞이면 hold 강등+flagged**, 일반 약어 AI/HBM 오탐 없음) + SYSTEM_PROMPT 인젝션 방어([NEWS]/[EXTERNAL]) + 다른 ticker/사이즈/한도 제안 금지 + macro_bias 사실블록 + `aget_advice` 비동기. ④ `researcher.py` 신규 — `decide_parallel`(종목당 1콜 `asyncio.gather(return_exceptions=True)` + retry 2 지수백오프 + 실패/깨진응답=hold 폴백 + 모든 symbol 키 보존, `advisor` 주입으로 테스트). ⑤ `daily_trader.py` — `_process_symbol`(직렬 단일) → 전종목 trend 1차수집(above_sma20 동시) → macro_bias N → decide_parallel → `select_picks`(conf≥80 BUY 상위 N + SELL, 동률 알파벳순 결정성) → 픽별 `_process_pick`(safety_gate→승인→Rank2). 종목/픽 단위 예외 격리·flagged audit. ⑥ 테스트: `test_sector`(33)/`test_llm_advisor`(17)/`test_researcher`(15)/`test_daily_trader`(10) 신규 + `test_universe_health.py`(서버 실행) + `test_safety_gate` 비화이트리스트 예시 NVDA→FAKE. `requirements.txt` tzdata(Windows zoneinfo) 추가. **잔여**: (a) 서버 `test_universe_health` 운영시간 실행 → 10종목 paper_tradable 실측(라이브 배포 전 선결), (b) approval.py 종목별 토글 다이제스트(현 per-pick 단건 승인으로 기능 대체), (c) 2주 라이브 운영 검증. ⚠️ 배포는 health 실측 후. |
| 2026-06-11 | **Phase 2 라이브 배포 완료(모의)**: 네이버클라우드 VM 재배포(`git pull` 66e80dc + systemd `kis-trader.service` restart). **배포전 게이트 2개 통과** — ① SMH 모의 일봉조회 OK(70캔들 ≥50, `bias=neutral`→N=2, macro_bias 실데이터 동작 확인 — SMH가 paper 에서 조회 안 됐으면 unknown→N=0 BUY 영구컷이었음) ② 서버 OpenAI 키 유효(모델 `gpt-5.5`, 401 아님 — 과거 sk-svcac 사고 재발 없음). 오프라인 6스위트 green(서버 Python 3.11.15; test_signals 는 셸 줄split 로 미실행이나 무변경 모듈), 서비스 active 단일 PID(3448796), `state.json` `_merge_defaults` 후방호환(open_orders/pending_orders 추가 + 기존 카운터 보존), audit 체인 OK(8줄). 기동 시 `telegram.error.Conflict`(stop→start 겹침 → 텔레그램 getUpdates 연결이 프로세스 종료 후 ~50s 서버측 잔류로 인한 일시현상) 발생했으나 **err 라인 377→377(60s 0증가) + pgrep 1개로 단일 폴러 확정 → 해소**(다른 호스트 2차 폴러 없음). 다음 07:30 KST 첫 11종목 사이클 대기. 런북·배포전 게이트·2주 검증 체크리스트(audit 이벤트 의미/red flags/합격기준 10): **docs/PHASE2_GO_LIVE.md**(배포준비 워크플로 3 에이전트 종합). 2주 운영 검증 진행 중. |
| 2026-06-11 | **Phase 2 커밋전 적대적 정밀리뷰 + 15건 수정(워크플로 23 에이전트, 6차원 리뷰→발견별 적대검증)**: 단위테스트가 못 닿은 라이브 통합부에서 확인된 실버그 전부 수정. **HIGH**: ① 거절/타임아웃/미체결 BUY 가 staged_buys 를 안 비워 같은 사이클 뒤 픽 cap 오염 → `Portfolio.unstage_buy` + 비체결 전 경로 해제. ② 일일 매수수/예산 cap 이 apply_fill(체결)에만 갱신돼 07:30 미체결 큐잉 사이클 내에서 무력 → `safety_gate.can_buy` 가 `staged_buys` 합산(exposure cap 과 동일 방식). ③ SELL 이 realized_pnl 을 안 넘겨 `daily_loss_realized_usd` 영영 0 → DAILY_LOSS_LIMIT 게이트 死 → `apply_fill` sell 평단대비 자동 산출. ④ 환각 정규식 `\b[A-Z]{2,5}\b` 가 한국어 조사 부착 ticker('NVDA가') 전부 놓침(SYSTEM_PROMPT 가 한국어 reason 강제 → 주류 형태) → ASCII lookaround `(?<![A-Za-z])...(?![A-Za-z])`. **MED**: ⑤ 주말/중복 pending 재큐잉 → ET 주말 가드 + `(symbol,side)` 미만료 pending dedup. ⑥ `consecutive_errors` 가 카운트만 되고 자동정지 미배선 → daily_cycle 진입 시 ≥3 이면 24h pause 무장(+submit 도 paused 존중). ⑦ breadth 가 AAPL(megacap_tech) 섞임 → 반도체 멤버만 집계. ⑧ SMH 20~49 캔들 시 sma50 None→unknown→N=0 모호 → 충분성 임계 50 으로 정렬. ⑨ OpenAI per-call timeout 미구현(§11 위반) → `OpenAI(timeout=30, max_retries=0)` + `decide_parallel` per_call_timeout `wait_for`. **LOW**: ⑩ action 공백 미strip, ⑪ confidence 문자열-소수('85.7')→0. 회귀 테스트 추가(`test_portfolio` 신규 + test_safety_gate 25/llm_advisor/researcher/sector 보강). 오프라인 7스위트 전부 green. |
